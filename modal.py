...
# pip install sentence-transformers datasets scikit-learn numpy
...
# Path to frontend's db-store.json (same directory as this script)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DB_STORE_PATH = os.path.join(SCRIPT_DIR, "public", "db-store.json")
# Also save to root for Vite static serving
...
# Also save to public/ if the directory exists (Vite public folder)
public_dir = os.path.join(SCRIPT_DIR, "public")
if os.path.isdir(public_dir):
    public_db_path = os.path.join(public_dir, "db-store.json")
    with open(public_db_path, "w", encoding="utf-8") as f:
        json.dump(db_store, f, ensure_ascii=False, indent=2)
    print(f"  [OK] Saved: {public_db_path}")
